...
namespace UnityGameFramework.Runtime
{
    ...
    public sealed partial class NetworkComponent : GameFrameworkComponent
    {
        ...
        [SerializeField]
        private List<NetworkChannel> m_NetworkChannels = new List<NetworkChannel>();
        ...
        public bool CreateAndSetChannel(string name, string ip, int port, int maxlength)
        {
            if (HasNetworkChannel(name))
                return false;
            NetworkChannel newChannel = new NetworkChannel();
            newChannel.Name = name;
            newChannel.HostOrIPString = ip;
            newChannel.Port = port;
            newChannel.MaxPacketLength = maxlength;
            newChannel.SetNetworkChannel(CreateNetworkChannel(name));
            newChannel.HeartBeatInterval = 100000;
            m_NetworkChannels.Add(newChannel);
            return true;
        }
        /// <summary>
        /// 检查是否存在网络频道。
        ...
        public bool DestroyNetworkChannel(string name)
        {
            ...
            {
                NetworkChannel Channel = m_NetworkChannels[i];
        ...
        public IPacketHandler RegisterHandler(IPacketHandler handler)
        {
            packetHandler = m_NetworkManager.RegisterHandler(handler);
            return packetHandler;
        }

        public void UnRegisterHandler(IPacketHandler handler)
        {
            m_NetworkManager.UnRegisterHandler(handler);
        }
