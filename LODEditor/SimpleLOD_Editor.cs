...
public class SimpleLOD_Editor : MonoBehaviour {
    public static SimpleLOD_EditorPopup window;
	[MenuItem ("Tools/SimpleLOD")]
	static void SimpleLOD () 
    {
		window = (SimpleLOD_EditorPopup)EditorWindow.GetWindow(typeof (SimpleLOD_EditorPopup));
		window.OpenWindow();
    }
    ...
    [MenuItem("GameObject/Video/一键删除LODSwitcher")]
    static void OneKeyDeleteLODSwitcher()
    {
        LODSwitcher[] lods = Selection.activeGameObject.GetComponentsInChildren<LODSwitcher>();
        for (int i = 0; i < lods.Length; i++)
        {
            DeleteLODSwitcherMeshs(lods[i]);
            UnityEngine.Object.DestroyImmediate(lods[i], true);
        }  
    }

    [MenuItem("GameObject/Video/一键删除LODMeshGroup")]
    static void OneKeyDeleteLODMeshGroup()
    {
        LODMeshGroup[] groups = Selection.activeGameObject.GetComponentsInChildren<LODMeshGroup>();
        for (int i = 0; i < groups.Length; i++)
        {
            UnityEngine.Object.DestroyImmediate(groups[i], true);
        }
    }

     public static void DeleteLODSwitcherMeshs(LODSwitcher lod)
     {
         for (int i = 0; i < lod.lodMeshes.Length; i++)
         {
             if (!lod.lodMeshes[i])
                 continue;
             if(lod.lodMeshes[i]==lod.orgMesh)
                 continue;
             string path = AssetDatabase.GetAssetPath(lod.lodMeshes[i]);
             AssetDatabase.DeleteAsset(path);
             AssetDatabase.Refresh();
         }
         Array.Clear(lod.lodMeshes, 0, lod.lodMeshes.Length);
     }


     [MenuItem("GameObject/Video/一键LOD/All")]
     static void OneKeyLOD()
     {
         GameObject[] groups = GameObject.FindGameObjectsWithTag("Group");
         if (groups == null || groups.Length == 0)
         {
             EditorUtility.DisplayDialog("警告", "没有能LOD的模型，请执行 场景配置检查操作", "Close");
             return;
         }
         SimpleLOD();
         for (int i = 0; i < groups.Length; i++)
         {
             OneKeyLODGroup(groups[i].transform,i);
         }
         EditorUtility.ClearProgressBar();
     }

     [MenuItem("GameObject/Video/一键LOD/LODGroup")]
     static void OneKeyLODGroup()
     {
         if (Selection.activeGameObject.tag != "Group")
         {
             EditorUtility.DisplayDialog("警告", "请选择 LODGroup", "Close");
             return;
         }
         SimpleLOD();
         OneKeyLODGroup(Selection.activeGameObject.transform);
     }

     static void OneKeyLODGroup(Transform parent, int index = 0)
     {
         LODMeshGroup meshgroup = parent.GetComponent<LODMeshGroup>();
         if (meshgroup)
         {
             window.nrOfLevels = meshgroup.LodLevels;
             window.compression = meshgroup.compress.Clone() as float[];
             window.smallObjectsValue = meshgroup.smallObjectsValue;
         }
         else
         {
             meshgroup = parent.gameObject.AddComponent<LODMeshGroup>();
             meshgroup.LodLevels = window.nrOfLevels;
             meshgroup.compress = window.compression.Clone() as float[];
             meshgroup.smallObjectsValue = window.smallObjectsValue;
         }
         for (int j = 0; j < parent.childCount; j++)
         {
             MeshLOD(parent.GetChild(j), meshgroup);
             EditorUtility.DisplayProgressBar("LOD中...", string.Format("Group{0} {1}/{2}",index, j, parent.childCount), (float)j / (float)parent.childCount);
         }
         EditorUtility.ClearProgressBar();
     }

     [MenuItem("GameObject/Video/一键LOD/LODMesh")]
     static void LODMesh()
     {
         if (Selection.activeGameObject.transform.childCount == 0)
             return;
         LODMeshGroup group = Selection.activeGameObject.GetComponent<LODMeshGroup>();
         if(!group)
         {
           bool sure=  EditorUtility.DisplayDialog("警告", "请确认对模型组 LOD", "Yes");
           if (!sure)
               return;
           SimpleLOD();
           group = Selection.activeGameObject.AddComponent<LODMeshGroup>();
           group.LodLevels = window.nrOfLevels;
           group.compress = window.compression.Clone() as float[];
           group.smallObjectsValue = window.smallObjectsValue;
         }
         else
         {
             SimpleLOD();
             window.nrOfLevels = group.LodLevels;
             window.compression = group.compress.Clone() as float[];
             window.smallObjectsValue = group.smallObjectsValue;
         }
         MeshLOD(Selection.activeGameObject.transform, group);
     }

     [MenuItem("GameObject/Video/一键LOD/LODMesh", true)]
     static bool ValidateLODMesh()
     {
         return Selection.activeGameObject.transform.childCount > 0;
     }

     static void MeshLOD(Transform parent, LODMeshGroup parentgroup)
     {
         LODSwitcher[] lods = parent.GetComponentsInChildren<LODSwitcher>();
         for (int i = 0; i < lods.Length; i++)
         {
             DeleteLODSwitcherMeshs(lods[i]);
             UnityEngine.Object.DestroyImmediate(lods[i], true);
         ...
         {
             window.nrOfLevels = parentgroup.LodLevels;
             window.compression = parentgroup.compress.Clone() as float[];
             window.smallObjectsValue = parentgroup.smallObjectsValue;
         }

     
         MeshFilter[] filters = parent.GetComponentsInChildren<MeshFilter>(true);
         LODSwitcher lod = SingleMeshLOD(filters[0]);
         if (!lod)
             return;
         if (UnityEditorInternal.ComponentUtility.CopyComponent(lod))
         {
             for (int i = 1; i < filters.Length; i++)
                 UnityEditorInternal.ComponentUtility.PasteComponentAsNew(filters[i].gameObject);
         }
     }

    static LODSwitcher SingleMeshLOD(MeshFilter filter)
    {
        Mesh mesh = filter.sharedMesh;
        string warning = SimpleLOD_EditorPopup.DurationWarning(mesh, true);
        if (warning == null || EditorUtility.DisplayDialog("Create LOD levels", warning, "Ok, let's go", "Cancel"))
        {
            string result = window.CreateLODSwitcherWithMeshes(filter.gameObject);
            UnityEngine.Debug.Log("Create LOD levels: " + result);
        }
        return filter.gameObject.GetComponent<LODSwitcher>();
    }
}
...
public class SimpleLOD_EditorPopup : EditorWindow {
    ...
    public int nrOfLevels = 3;
    public float[] compression = new float[5] { 0.25f, 0.5f, 1f, 1.5f, 2f };
    public float smallObjectsValue = 1f;
	...
	public static string MergeChildMeshesPerMaterial(GameObject aGo) {
	...
	public string CreateLODSwitcherWithMeshes(GameObject aGo) {
		if(aGo == null) return "No gameObject selected";
		...
		if(path != null) {
			...
			if(meshes != null) {
                ...
                float count = meshes[meshes.Length-1].triangles.Length / 3;
                float totalcount = meshes[0].triangles.Length / 3;

				for(int i=1;i<meshes.Length;i++) {
                    if (count <= 10 || (count / totalcount)>=0.75f)
                    {
                        sizeStr = string.Format("LOD优化异常 优化后面数{0}，实际面数{1}", count, totalcount);
                        Debug.LogError(sizeStr);
                        LODSwitcher ls = aGo.GetComponent<LODSwitcher>();
                        SimpleLOD_Editor.DeleteLODSwitcherMeshs(ls);
                        UnityEngine.Object.DestroyImmediate(ls, true);
                        return sizeStr;
                    }
	...
	public static string StoragePathUsing1stMeshAndSubPath(GameObject aGo, string subPath) {
	...
	public static string DurationWarning(Mesh mesh, bool compress) {
		int vertexCount = 0;
