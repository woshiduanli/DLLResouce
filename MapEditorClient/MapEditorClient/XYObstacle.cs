...
public class XYObstacleLineList : IEnumerable<XYObstacleLine>, IXYBinarySerializable {
	...
	List<XYObstacleLine> list_;
    public Dictionary<XYGridPos, short> PointList;
	...
	public XYObstacleLineList(int capacity) {
        ...
        PointList = new Dictionary<XYGridPos, short>();
	}
	...
	public bool Add(XYObstacleLine line) {
		if (CanAdd(line)) {
            ...
            {
                PointList[line.Begin] += 1;
            }
            else
            {
                PointList.Add(line.Begin,1);
            }
            if (PointList.ContainsKey(line.End))
            {
                PointList[line.End] += 1;
            }
            else
            {
                PointList.Add(line.End, 1);
            }
			return true;
		} else {
			return false;
	...
	public bool RemoveLast() {
		if (list_.Count > 0) {
            ...
            {
                PointList[list_[list_.Count - 1].Begin] -= 1;
            }
            if (PointList.ContainsKey(list_[list_.Count - 1].End))
            {
                PointList[list_[list_.Count - 1].End] -= 1;
            }
