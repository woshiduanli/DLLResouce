...
namespace SimpleSpritePackerEditor
{
    ...
    {
        private string searchSP = string.Empty;
        ...
        private void DrawActionButtons()
        {
            ...
            if (GUI.Button(controlRect, "Clear Atlas", EditorStyles.miniButton))
            {
            ...
            controlRect.x = EditorGUILayout.GetControlRect().x;
            if (GUI.Button(controlRect, "更 换 图 片", EditorStyles.miniButton))
            {
                Dictionary<string, Texture> texdic = new Dictionary<string, Texture>();
                Selection.activeObject = AssetDatabase.LoadAssetAtPath("Assets/Resources/UI/Sprite", typeof(UnityEngine.Object));
                UnityEngine.Object[] texs = Selection.GetFiltered(typeof(Texture), SelectionMode.DeepAssets);
                for (int i = 0; i < texs.Length; i++)
                    texdic[texs[i].name] = texs[i] as Texture;

                List<SPSpriteInfo> sprites = this.m_SPInstance.copyOfSprites;
                List<string> deletes = new List<string>();
                foreach (SPSpriteInfo info in sprites)
                {
                    if (texdic.ContainsKey(info.name))
                    {
                        deletes.Add(info.name);
                        this.m_SPInstance.QueueAction_RemoveSprite(info);
                        this.m_SPInstance.QueueAction_AddSprite(texdic[info.name]);
                    }
                }
                Selection.activeObject = this.target;
                //this.m_AtlasBuilder.RebuildAtlas();
                //foreach (string iname in deletes)
                //{
                //    this.m_SPInstance.QueueAction_AddSprite(texdic[iname]);
                //}
                //Selection.activeObject = this.target;
            }

            controlRect.x = controlRect.width + 24f;
            if (GUI.Button(controlRect, "删 除 无 用 图 片", EditorStyles.miniButton))
            {
                ...
                for (int i = 0; i < uguis.Length; i++)
                {
                    ...
                    {
                        GameObject ui = uguis[i] as GameObject;
                        EditorUtility.DisplayProgressBar("查找UI中...", string.Format("UI: {0} {1}/{2}", ui.name, i, uguis.Length), (float)i / (float)uguis.Length);

                        CImage[] images = ui.GetComponentsInChildren<CImage>(true);
                        for (int j = 0; j < images.Length; j++)
                        {
                            CImage ci = images[j];
                            EditorUtility.DisplayProgressBar("查找CImage中...", string.Format("CImage: {0} {1}/{2}", ci.name, j, images.Length), (float)j / (float)images.Length);

                            if (ci.AtlasName != this.target.name || string.IsNullOrEmpty(ci.AtlasName))
                                continue;
                            splist.Add(ci.SpriteName);
                        }
                    }
                }

                controlRect.x += 24;
                if (GUI.Button(controlRect, "图集清理", EditorStyles.miniButton))
                {
            ...
            controlRect.y += 24;
            GUI.Label(controlRect, "查找图片");
            controlRect.x += 70;
            searchSP = GUI.TextField(controlRect, searchSP);
            controlRect.x = controlRect.width + 100;
        ...
        private void DrawSpritesWithThumbs()
        {
            ...
            foreach (SPSpriteInfo info in sprites)
            {
                ...
                {
                    if (info.name != searchSP && !info.name.Contains(searchSP))
                        continue;
                }
                ...
                case EventType.DragPerform:
                    {
                        ...
                        if (evt.type == EventType.DragPerform)
                        {
                            ...
                            {
                                if (this.m_SPInstance.sprites.Find(s => s.name == filtered[i].name) != null)
                                {
