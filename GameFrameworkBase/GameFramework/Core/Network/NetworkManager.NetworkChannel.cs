        ...
        private sealed partial class NetworkChannel : INetworkChannel, IDisposable
        {
            ...
            public void Send<T>(T packet, object userData) where T : Packet
            {
                ...
                try
                {
                    ...
                    packetLength = (ushort)(length - ReceiveState.HeaderLength);
                    Utility.Converter.GetBytesFromShort(IPAddress.HostToNetworkOrder((short)packetLength)).CopyTo(packetBuffer, 0);
            ...
            private bool Process()
            {
                ...
                if (m_ReceiveState.Length == ReceiveState.HeaderLength)
                {
                    ushort packetLength = Utility.Converter.GetUShortFromBytes(m_ReceiveState.GetBuffer());
