        ...
        public static void DeleteLODSwitcherMeshs(LODSwitcher lod)
        {
            for (int i = 0; i < lod.lodMeshes.Length; i++)
            {
                if (lod.lodMeshes[i] == lod.orgMesh)
                    continue;
                string path =UnityEditor.AssetDatabase.GetAssetPath(lod.lodMeshes[i]);
                UnityEditor.AssetDatabase.DeleteAsset(path);
                UnityEditor.AssetDatabase.Refresh();
            }
            UnityEngine.Object.DestroyImmediate(lod, true);
        }
		...
		public static Mesh[] SetUpLODLevelsWithLODSwitcher(this GameObject go, float[] lodScreenSizes, float[] maxWeights, bool recalcNormals, float removeSmallParts = 1f, float protectNormals = 1f, float protectUvs = 1f, float protectSubMeshesAndSharpEdges = 1f, float smallTrianglesFirst = 1f, int nrOfSteps = 1) {
			...
			if(lodSwitcher != null) {
                ...
                if (Application.isEditor)
                    DeleteLODSwitcherMeshs(lodSwitcher);
			}
